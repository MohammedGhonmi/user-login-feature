<section>
    ...
    <form method="post" enctype="multipart/form-data" action="{{ route('user.update') }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')

        <div>
            <x-input-label for="prefixname" :value="__('Prefixname')" />
            <select name="prefixname" class="block mt-1 w-full">
                @foreach (['Mr', 'Mrs', 'Ms'] as $prefixname)
                    <option value="{{ $prefixname }}" @selected(old('prefixname', $user->prefixname) == $prefixname)>
                        {{ $prefixname }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <x-text-input id="photo" name="image" type="file" class="mt-1 block w-full" />
        </div>
